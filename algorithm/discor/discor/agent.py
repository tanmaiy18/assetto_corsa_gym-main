...
class Agent:
    ...
    def load(self, path, load_buffer=True):
        buffer_path = path + "replay_buffer.pkl"
        if load_buffer and os.path.exists(buffer_path):
            with open(buffer_path, "rb") as f:
                self.memory = pickle.load(f)
        else:
            print("Skipping replay buffer load (file not found or load_buffer=False)")
    ...
    def run(self):
        try:
            ...
            self.save(
                ...
                save_buffer=self.save_final_buffer,
            )
    ...
    def train_episode(self):
        ...
        try:
            ...
            while not done:
                ...
                self._replay_buffer.append(
                    state, action, reward, next_state, masked_done, episode_done=rb_done
                )
                ...
                if self.checkpoint_freq and (self._steps % self.checkpoint_freq == 0):
                    ...
                    self.save(
                        os.path.join(
                            self._model_dir, "checkpoints", f"step_{self._steps:08d}"
                        ),
                        save_buffer=False,
                    )
        except TimeoutError:
        ...
        logger.info(
            f"Avr step time: {eval_metrics['step_perf_mean']:.3f}s, actions: {eval_metrics['action_perf_mean']:.4f}s, update: {eval_metrics['update_model_perf_mean']:.3f}s"
        )
        logger.info(
            f"Max step time: {eval_metrics['step_perf_max']:.3f}s, actions: {eval_metrics['action_perf_max']:.4f}s, update: {eval_metrics['update_model_perf_max']:.3f}s"
        )
        logger.info(
            f"std step time: {eval_metrics['step_perf_std']:.3f}s, actions: {eval_metrics['action_perf_std']:.4f}s, update: {eval_metrics['update_model_perf_std']:.3f}s"
        )
        logger.info(
            f"step_perf_> thres: {eval_metrics['step_perf_> thres']} / {len(step_perf)}"
        )
        ...
        pd.DataFrame(self.episodes_stats).to_csv(
            os.path.join(self._log_dir, "summary.csv"), index=None
        )
        logger.info(
        ...
        try:
            ...
            for _ in range(self._num_eval_episodes):
                ...
                while not done:
                    ...
                    self._test_env.states[-1]["entropies"] = (
                        entropies.cpu().numpy().item()
                    )
                    episode_return += reward
                    state = next_state
            ...
            env_ep_stats = self._env.close()
            pd.DataFrame([env_ep_stats]).to_csv(
                os.path.join(self._log_dir, "eval_summary.csv"), index=None
            )
        ...
        for ep in tqdm(range(env_data.trajectories_count)[:]):
            ...
            for i in range(len(env_data.trajectory) - 1):
                ...
                if i >= len(env_data.trajectory) - 2:
                    ...
                    episode_done = (
                        False  # use the termination signal from the environment
                    )
