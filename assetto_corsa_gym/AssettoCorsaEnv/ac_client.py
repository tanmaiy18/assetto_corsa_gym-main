...
class Client:
    def __init__(self, config):
        ...
        self.simulation_management_server_host_name = (
            config.simulation_management_server_host_name
        )
        self.simulation_management_server_port = (
            config.simulation_management_server_port
        )
        ...
        if self.screen_capture_enable:
            self.camera = dual_buffer.GrabberSharedMemoryDualBuffer(
    ...
    def setup_connection(self):
        ...
        logger.info(
            f"AC Client. Listening at host: {self.server_host} port: {self.server_port}"
        )
    ...
    def step_sim(self):
        ...
        if (
            self.record_controls_from_client
            and self.joystick_handler
            and self.joystick_handler.is_initialized()
        ):
    ...
    def export_track_and_racing_line(self, output_path="."):
        ...
        df = pd.DataFrame({"pos_x": fast_lane_x, "pos_y": fast_lane_y})
...
class ServerState(dict):
    ...
    def parse_server_str(self, server_string):
        self.update(
            eval(
                server_string,
                {"NaN": float("nan"), "Inf": float("inf"), "nan": float("nan")},
            )
        )
        ...
        # Replace any NaN values with 0 to prevent downstream errors
        import math
...
class DriverControls(dict):
    ...
    def apply_local_controls(self):
        self.local_controls.set_controls(
            steer=self["steer"],
            acc=self["acc"],
            brake=self["brake"],
