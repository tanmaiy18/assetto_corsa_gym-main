...
RACING_LINE_DIAMETER_CROP = 300.0
PAST_ACTIONS_WINDOW = 3

# Curvature look ahead
CURV_LOOK_AHEAD_DISTANCE = 300.0  # METERS
...
# episode termination
TERMINAL_LIMIT_PROGRESS_SPEED = (
    1.0  # [m/s], episode terminates if car is running slower than this limit
)
TERMINAL_JUDGE_TIMEOUT = 10.0  # If after this number of seconds still no progress, terminate the episode. In seconds
...
class TaskIdsIndexer:
    ...
    def get_index(self, track, car):
        ...
        key = f"{track}_{car}"
        return self.combination_indices.get(
            key, -1
        )  # Return -1 if the combination does not exist
...
class AssettoCorsaEnv(Env, gym_utils.EzPickle):
    ...
    obs_channels_info = {
        ...
        "tyre_slip_ratio_fl": 1.0,
        "tyre_slip_ratio_fr": 1.0,
        "tyre_slip_ratio_rl": 1.0,
        "tyre_slip_ratio_rr": 1.0,
        "Dy_rr": 2.0,  # lateral grip
        "Dy_rl": 2.0,  # very noisy. Goes to zero when the wheel is spinning
        "Dy_fr": 2.0,
        "Dy_fl": 2.0,
        "LapCount": 1.0,
        "LapDist": 1.0,
    ...
    def __init__(
        ...
        max_gap=None,
        gap_const=12.0,  # gap penalization
        penalize_gap=True,
        ...
        self.enable_out_of_track_termination = (
            self.config.enable_out_of_track_termination
        )
        ...
        self.enable_out_of_track_penalty = (
            self.config.enable_out_of_track_penalty
        )  # oot penalization in the reward function
        ...
        self.track = Track(
            track_file_path=self.track_file,
            track_grid_file=self.track_grid_file,
            torch_device=None,
        )  # Note: send the torch device to find the grid using pytorch

        #
        self.ref_lap = ReferenceLap(
            self.ref_lap_file, use_target_speed=self.use_target_speed
        )
        self.racing_line = self.ref_lap.get_racing_line_time()
        if torch_device:
            self.racing_line_torch = torch.tensor(
                self.racing_line, device=torch_device, dtype=torch.float
            )
        ...
        if self.enable_sensors:
            self.sensors = sensors_ray_casting.SensorsRayCasting(
                self.track.right_border_x,
                self.track.right_border_y,
                self.track.left_border_x,
                self.track.left_border_y,
            )
        ...
        self.controls_rate_limit = np.array(
            [
        ...
        self.adjusted_controls_rate_limit = (
            self.controls_rate_limit / self.controls_scale_factor[:, np.newaxis]
        )
        ...
        if self.use_obs_extra:
            self.obs_extra_shape = (
                len(self.obs_extra_enabled_channels),
            )  # (features,)
        else:
        ...
        if verbose:
            ...
            logger.info(
                f"enable_out_of_track_termination: {self.enable_out_of_track_termination}"
            )
            logger.info(
                f"enable_low_speed_termination: {self.enable_low_speed_termination}"
            )
            logger.info(
                f"enable_out_of_track_penalty: {self.enable_out_of_track_penalty}"
            )
            ...
            if self.config.enable_task_id_in_obs:
                logger.info(
                    f"task_id: {self.current_task_id} num_tasks: {self.tasks_ids.get_number_of_tasks()}"
                )
        ...
        self.obs_channels_scales = np.array(
            [self.obs_channels_info[ch] for ch in self.obs_enabled_channels]
        )
        if verbose:
    ...
    def update_racing_line(self, x, y):
        ...
        self.racing_line[
            np.sqrt(
                (self.racing_line[:, 0] - x) ** 2 + (self.racing_line[:, 1] - y) ** 2
            )
            < RACING_LINE_DIAMETER_CROP
        ]
        if self.torch_device:
            self.racing_line_torch = torch.tensor(
                self.racing_line, device=self.torch_device, dtype=torch.float
            )
    ...
    def preprocess_actions(self, actions, current_actions):
        if self.use_relative_actions:
            ...
            new_actions = (
                current_actions + actions * self.adjusted_controls_rate_limit[:, 1]
            )
        else:
            new_actions = actions
    ...
    def inverse_preprocess_actions(self, prev_abs_actions, current_abs_actions):
        if self.use_relative_actions:
            ...
            return np.clip(
                current_abs_actions, self.controls_min_values, self.controls_max_values
            )
    ...
    def set_actions(self, actions):
        ...
        self.client.controls.set_controls(
            steer=self.actions[0], acc=self.actions[1], brake=self.actions[2]
        )
    ...
    def step(self, action=None):
        ...
        if (self.ep_steps % 50) == 0:
            logger.debug(
                f"t: {self.ep_steps} speed: {state['speed']:.2f}, oot: {state['out_of_track']} "
    ...
    def expand_state(self, state):
        state["currentTime"] = state["currentTime"] / 1000.0
        ...
        self.update_racing_line(
            x, y
        )  # get a smaller version to decrease gap computation time
        ...
        if self.enable_out_of_track_calculation:
            state["out_of_track_calc"] = (
                0.0
                if self.track.track_occupancy_grid.is_inside_grid(point).item() > 0.0
                else 1.0
            )
            state["out_of_track_ac"] = 1.0 if state["numberOfTyresOut"] > 2.0 else 0.0
        ...
        if self.enable_sensors:
            ...
            if state["out_of_track_calc"]:
                ...
                state["dist_to_border"] = min(
                    state["dist_to_border"], new_sensors.min()
                )

        # TODO implement if needed
        state["going_backwards"] = 0.0
        ...
        if self._max_episode_steps is not None:
            if self.ep_steps > self._max_episode_steps:
                logger.info(
                    f"Terminate episode. Max steps {self.ep_steps}/{self._max_episode_steps}"
                )
                done = 1
                buf_infos["TimeLimit.truncated"] = True

        if (
            self.max_laps_number
            and self.get_lap_count(self.states) > self.max_laps_number
        ):
            logger.info(
                f"Terminate episode. Max number of laps reached {self.max_laps_number}"
            )
            done = 1
            buf_infos["TimeLimit.truncated"] = True
        ...
        if (
            self.enable_low_speed_termination
            and state["speed"] < TERMINAL_LIMIT_PROGRESS_SPEED
        ):
    ...
    def get_extra_observations(self, state):
        return np.array(
            [state[channel] for channel in self.obs_extra_enabled_channels],
            dtype=np.float32,
        )
    ...
    def reset(self, seed=None, verbose=False):
        ...
        self.n_episodes += 1
        logger.info(
            f"Reset AC. Episode {self.n_episodes} total_steps: {self.total_steps}"
        )
        ...
        if self.n_episodes == 1:
            ...
            if self.config.screen_capture_enable:
                assert (
                    ...
                    "Config and AC Mod config do not match for final_image_height. Got {} and {}".format(
                        self.config.final_image_height,
                        self.ac_mod_config["final_image_height"],
                    )
                )
                assert (
                    self.config.final_image_width
                    == self.ac_mod_config["final_image_width"]
                ), (
                    "Config and AC Mod config do not match for final_image_width. Got {} and {}".format(
                        self.config.final_image_width,
                        self.ac_mod_config["final_image_width"],
                    )
                )
                assert self.config.color_mode == self.ac_mod_config["color_mode"], (
                    "Config and AC Mod config do not match for color_mode. Got {} and {}".format(
                        self.config.color_mode, self.ac_mod_config["color_mode"]
                    )
                )

            assert (
                self.config.ego_sampling_freq == self.ac_mod_config["ego_sampling_freq"]
            ), "Ego sampling frequency mismatch"
            assert self.static_info["TrackFullName"] == self.track_name, (
                f"Track name mismatch. Running: {self.static_info['TrackFullName']} Configured: {self.track_name}"
            )
            assert self.static_info["CarName"] == self.car_name, (
                f"Track name mismatch. Running: {self.static_info['CarName']} Configured: {self.car_name}"
            )
    ...
    def get_obs(self, state, history=None):
        ...
        LAC = self.ref_lap.get_curvature_segment(
            state["LapDist"],
            CURV_LOOK_AHEAD_DISTANCE,  # meters ahead
            CURV_LOOK_AHEAD_VECTOR_SIZE,
        )  # size of the vector downsampled
        ...
        if len(history) < PAST_ACTIONS_WINDOW:
            ...
            current_controls_steer_prev = np.array(
                [
                    history[i]["steerAngle"] / self.obs_channels_info["steerAngle"]
                    for i in range(-PAST_ACTIONS_WINDOW, 0)
                ]
            )  # if PAST_ACTIONS_WINDOW=3; -3, -2, -1
            current_controls_pedal_prev = np.array(
                [history[i]["accStatus"] for i in range(-PAST_ACTIONS_WINDOW, 0)]
            )
            current_controls_brake_prev = np.array(
                [history[i]["brakeStatus"] for i in range(-PAST_ACTIONS_WINDOW, 0)]
            )
            obs = np.hstack(
                [
                    obs,
                    current_controls_steer_prev,
                    current_controls_pedal_prev,
                    current_controls_brake_prev,
                ]
            )
            actions_diff = np.array(
                [
                    (state["steerAngle"] - history[-1]["steerAngle"])
        ...
        obs = np.hstack(
            [
                obs,
                state["steerAngle"] / self.obs_channels_info["steerAngle"],
                state["accStatus"],
                state["brakeStatus"],
            ]
        )
        ...
        if self.add_previous_obs_to_state:
            if len(history) < PAST_ACTIONS_WINDOW:
                prev_obs = np.zeros(
                    PAST_ACTIONS_WINDOW * self.previous_obs_to_state_dim
                )
            else:
                prev_obs = []
                for i in range(-PAST_ACTIONS_WINDOW, 0):
    ...
    def get_sensors(self, state, do_plots=False):
        intersections, vector_lengths, filtered_walls, points, rectangle = (
            self.sensors.get_intersections_and_distances_to_wall(
                state["world_position_x"], state["world_position_y"], state["yaw"]
            )
        )
        if do_plots:
            sensors_ray_casting.make_plots(
                self.sensors.scene,
                filtered_walls,
                points,
                intersections,
                state["world_position_x"],
                state["world_position_y"],
                rectangle,
                self.sensors.number_of_rays,
            )
        ...
        if len(self.states) and self.stats_saved is False:
            ...
            if self.save_observations:
                ...
                if save_csv:
                    ...
                    ep[save_csv_channels].to_csv(
                        f"{timestamp}_raw_data.csv", index=False
                    )
            ...
            r = {
                ...
                "BestLap": ep["BestLap"].values[-1] / 1000.0,
                "terminated": ep.terminated.values[-1],
            }
            for i, lapCount in enumerate(list(set(ep.LapCount))):
                r[f"LapNo_{i}"] = (
                    ep[ep.LapCount == lapCount]["iLastTime"].values[-1] / 1000
                )  # to seconds
            #  BestLap from the dictionary, excluding lap times that are 0 (incomplete laps)
            r["ep_bestLapTime"] = min(
                (
                    time
                    for key, time in r.items()
                    if key.startswith("LapNo_") and time > 0
                ),
                default=0,
            )
            if verbose:
                logger.info(
                    f"total_steps: {self.total_steps} ep_steps: {self.ep_steps} ep_reward: {r['ep_reward']:6.1f} "
                ...
                if len(ep) > 10:
                    ...
                    logger.info(
                        f"dt avr: {dt.mean():.3f} std: {dt.std():.3f} min: {dt.min():.3f} max: {dt.max():.3f}"
                    )
            ...
            pd.DataFrame(self.episodes_stats).to_csv(
                self.output_path + os.sep + "episodes_stats.csv", index=False
            )
    ...
    def load_history(self, file_path):
        ...
        if file_path.suffix == ".parquet":
            trajectory = pd.read_parquet(file_path, engine="pyarrow").to_dict(
                orient="records"
            )
            ...
            assert static_info_path.exists(), (
                f"Static info file not found: {static_info_path}"
            )
    ...
    def set_track(self, track_name):
        ...
        self.track_file = os.path.join(
            self.tracks_path, self.track_config["track_file"]
        )
        self.ref_lap_file = os.path.join(
            self.tracks_path, self.track_config["ref_lap_file"]
        )
        self.track_grid_file = os.path.join(
            self.tracks_path, self.track_config["track_grid_file"]
        )
