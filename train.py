...
from discor.anchored_sac import AnchoredDisCor
...
def parse_args(hardcode=None):
    ...
    parser.add_argument(
        "--config", default="config.yml", type=str, help="Path to configuration file"
    )
    parser.add_argument(
        "--load_path",
        type=str,
        default=None,
        help="Path to load the model from (default: None)",
    )
    parser.add_argument(
    ...
    args.load_path = (
        os.path.abspath(args.load_path) + os.sep if args.load_path is not None else None
    )
    return args
...
def main():
    ...
    if args.algo == "anchored_discor":
        algo = AnchoredDisCor(
            ...
            **OmegaConf.to_container(config.get("Anchoring", {})),
        )
    elif args.algo == "discor":
        algo = DisCor(
            state_dim=env.observation_space.shape[0],
            action_dim=env.action_space.shape[0],
            device=device,
